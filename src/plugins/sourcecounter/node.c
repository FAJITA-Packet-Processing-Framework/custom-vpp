...
typedef struct
{
  ...
  u16 src_port;
  u16 dst_port;
} sourcecounter_trace_t;


/* packet trace format function */
static u8 *
format_sourcecounter_trace (u8 * s, va_list * args)
{
  CLIB_UNUSED (vlib_main_t * vm) = va_arg (*args, vlib_main_t *);
  CLIB_UNUSED (vlib_node_t * node) = va_arg (*args, vlib_node_t *);
  sourcecounter_trace_t *t = va_arg (*args, sourcecounter_trace_t *);

  s = format (s, "SOURCECOUNTER: sw_if_index %d, next index %d\n",
...
#define foreach_sourcecounter_error \
_(SWAPPED, "Mac swap packets processed") \
_(INSERTS, "Packets inserted")

typedef enum
{
#define _(sym,str) SOURCECOUNTER_ERROR_##sym,
  foreach_sourcecounter_error
#undef _
    SOURCECOUNTER_N_ERROR,
} sourcecounter_error_t;

static char *sourcecounter_error_strings[] = {
...
typedef enum
{
  SOURCECOUNTER_NEXT_INTERFACE_OUTPUT,
  SOURCECOUNTER_N_NEXT,
} sourcecounter_next_t;
...
VLIB_NODE_FN (sourcecounter_node) (vlib_main_t * vm, vlib_node_runtime_t * node,
			    vlib_frame_t * frame)
{
  u32 n_left_from, *from, *to_next;
  sourcecounter_next_t next_index;
  u32 thread_index = 0;
  clib_bihash_16_8_t hash_table = sourcecounter_main.per_cpu[thread_index].hash_table;
  ...
  while (n_left_from > 0)
    {
	  ...
	  while (n_left_from >= 8 && n_left_to_next >= 2){
		u32 next0 = SOURCECOUNTER_NEXT_INTERFACE_OUTPUT;
	  	u32 next1 = SOURCECOUNTER_NEXT_INTERFACE_OUTPUT;
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	  {
	    ...
	    {
	      sourcecounter_trace_t *t = vlib_add_trace (vm, node, b0, sizeof (*t));
  ...
  vlib_node_increment_counter (vm, sourcecounter_node.index,
			       SOURCECOUNTER_ERROR_SWAPPED, pkts_swapped);
  vlib_node_increment_counter (vm, sourcecounter_node.index,
			       SOURCECOUNTER_ERROR_INSERTS, pkts_inserted);
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (sourcecounter_node) =
{
  .name = "sourcecounter",
  .vector_size = sizeof (u32),
  .format_trace = format_sourcecounter_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,

  .n_errors = ARRAY_LEN(sourcecounter_error_strings),
  .error_strings = sourcecounter_error_strings,

  .n_next_nodes = SOURCECOUNTER_N_NEXT,

  /* edit / add dispositions here */
  .next_nodes = {
    [SOURCECOUNTER_NEXT_INTERFACE_OUTPUT] = "flowcounter",
  },
};
/* *INDENT-ON* */
