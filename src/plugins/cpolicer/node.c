...
static u8 *
format_cpolicer_trace (u8 * s, va_list * args)
{
  CLIB_UNUSED (vlib_main_t * vm) = va_arg (*args, vlib_main_t *);
  CLIB_UNUSED (vlib_node_t * node) = va_arg (*args, vlib_node_t *);
  cpolicer_trace_t *t = va_arg (*args, cpolicer_trace_t *);

  s = format (s, "POLICER: sw_if_index %d, next index %d\n",
...
extern vlib_node_registration_t cpolicer_node;

#define foreach_cpolicer_error \
_(SWAPPED, "Mac swap packets processed") \
_(INSERTS, "Packets inserted")

typedef enum
{
#define _(sym,str) POLICER_ERROR_##sym,
  foreach_cpolicer_error
#undef _
    POLICER_N_ERROR,
} cpolicer_error_t;

static char *cpolicer_error_strings[] = {
...
VLIB_NODE_FN (cpolicer_node) (vlib_main_t * vm, vlib_node_runtime_t * node,
			    vlib_frame_t * frame)
{
  u32 n_left_from, *from, *to_next;
  cpolicer_next_t next_index;
  u32 thread_index = 0;
  clib_bihash_16_8_t hash_table = cpolicer_main.per_cpu[thread_index].hash_table;
  ...
  while (n_left_from > 0)
    {
	  ...
	  while (n_left_from >= 8 && n_left_to_next >= 2){
		u32 next0 = POLICER_NEXT_INTERFACE_OUTPUT;
	  	u32 next1 = POLICER_NEXT_INTERFACE_OUTPUT;
		...
		clib_spinlock_lock (&cpolicer_main.writer_lock);

		if (clib_bihash_search_inline_with_hash_16_8 (&cpolicer_main.per_cpu[thread_index].hash_table, hash0, &value0) < 0) {
			key0.value = 1;
			pkts_inserted += 1;
		}
		else
		...
		clib_bihash_add_del_with_hash_16_8(&cpolicer_main.per_cpu[thread_index].hash_table, &key0, hash0, 1);

		if (clib_bihash_search_inline_with_hash_16_8 (&cpolicer_main.per_cpu[thread_index].hash_table, hash1, &value1) < 0) {
			key1.value = 1;
			pkts_inserted += 1;
		}
		else
		...
		clib_bihash_add_del_with_hash_16_8(&cpolicer_main.per_cpu[thread_index].hash_table, &key1, hash1, 1);

		clib_spinlock_unlock (&cpolicer_main.writer_lock);
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	  {
		...
		cpolicer_main_t * fcm = &cpolicer_main;
	    ...
	    {
	      cpolicer_trace_t *t = vlib_add_trace (vm, node, b0, sizeof (*t));
  ...
  vlib_node_increment_counter (vm, cpolicer_node.index,
			       POLICER_ERROR_SWAPPED, pkts_swapped);
  vlib_node_increment_counter (vm, cpolicer_node.index,
			       POLICER_ERROR_INSERTS, pkts_inserted);
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE (cpolicer_node) =
{
  .name = "cpolicer",
  .vector_size = sizeof (u32),
  .format_trace = format_cpolicer_trace,
  .type = VLIB_NODE_TYPE_INTERNAL,

  .n_errors = ARRAY_LEN(cpolicer_error_strings),
  .error_strings = cpolicer_error_strings,

  .n_next_nodes = POLICER_N_NEXT,

  /* edit / add dispositions here */
  .next_nodes = {
    [POLICER_NEXT_INTERFACE_OUTPUT] = "sourcecounter",
  },
};
/* *INDENT-ON* */
